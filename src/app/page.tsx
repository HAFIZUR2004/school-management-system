...
import { ArrowRight, Phone, BookOpen, PenTool, Rocket, Globe } from "lucide-react"; // নতুন আইকন
...
const deptCards = [
  {
    ...
    titleBn: "বিজ্ঞানাগার",
    descBn: "আধুনিক যন্ত্রপাতি সমৃদ্ধ ল্যাব যেখানে হাতে-কলমে শেখার সুযোগ রয়েছে।",
...
const schoolSchedule = [
  { nameBn: "অ্যাসেম্বলি", nameEn: "Assembly", time: "08:00 AM", tagBn: "বাধ্যতামূলক", tagEn: "Mandatory" },
  { nameBn: "১ম পিরিয়ড", nameEn: "1st Period", time: "08:30 AM", tagBn: "একাডেমিক", tagEn: "Academic" },
  { nameBn: "টিফিন বিরতি", nameEn: "Tiffin Break", time: "11:00 AM", tagBn: "বিশ্রাম", tagEn: "Break" },
  { nameBn: "২য় পিরিয়ড", nameEn: "2nd Period", time: "11:30 AM", tagBn: "একাডেমিক", tagEn: "Academic" },
  { nameBn: "স্পোর্টস/ক্লাব", nameEn: "Sports/Club", time: "01:30 PM", tagBn: "সহ-শিক্ষা", tagEn: "Co-Curricular" },
  { nameBn: "ছুটি", nameEn: "Dismissal", time: "02:30 PM", tagBn: "বাস", tagEn: "Home" },
];
...
export default function Home() {
  ...
  return (
    <div className="space-y-24 pb-20 bg-slate-50/50">
      {/* Hero Section */}
     <section className="relative min-h-[90vh] lg:min-h-screen flex items-center overflow-hidden bg-white mt-16 lg:mt-0">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div
          ...
          style={{
            clipPath: "polygon(0 0, 100% 0, 100% 40%, 65% 55%, 45% 90%, 0 100%)",
          }}
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-block px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-xs font-bold uppercase tracking-widest">
              {language === "bn" ? "ভর্তি চলছে - ২০২৬" : "Admissions Open - 2026"}
            </div>
            ...
            <div className="flex flex-wrap gap-8 pt-4 items-center">
              ...
              <div className="flex items-center gap-3 group cursor-pointer">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-white/70 uppercase tracking-wider italic">Call For More Info</span>
                  <span className="text-lg lg:text-2xl font-black text-white tracking-tighter italic group-hover:text-[#fbbf24] transition-colors">01234 567 890</span>
                </div>
              </div>
            </div>
          ...
          <div className="relative h-[400px] lg:h-[700px] flex items-center justify-center">
             ...
             <div className="relative w-full h-full">
                {floatingElements.map((item, index) => (
                    ...
                    >
                        {/* আইকনের সাইজ */}
                        <item.icon size={80} strokeWidth={1.5} className="lg:scale-150" />
                    </motion.div>
                ))}
             </div>
             ...
             >
                <div className="text-center text-[#fbbf24] space-y-2">
                    ...
                    <p className="text-3xl font-black uppercase tracking-tighter">২০২৬</p>
                    <p className="text-xs font-bold uppercase text-white/80">ভর্তি</p>
                </div>
             </motion.div>
          ...
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                {language === "bn" ? stat.key === 'students' ? 'ছাত্র-ছাত্রী' : stat.key === 'teachers' ? 'শিক্ষক' : stat.key === 'labs' ? 'ল্যাব' : 'সাফল্য' : stat.key}
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-slate-900 rounded-[3rem] p-8 md:p-12 border border-slate-800 shadow-2xl relative overflow-hidden text-white"
        >
          <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
            <div className="space-y-1 text-center md:text-left">
              ...
              <div className="flex items-center justify-center md:justify-start gap-2 text-slate-400 font-bold">
                ...
                <span className="text-sm uppercase tracking-widest">Academic Routine 2026</span>
              </div>
            </div>
            <div className="px-8 py-3 bg-white/5 rounded-2xl border border-white/10 text-amber-400 font-bold">
              {new Date().toLocaleDateString(language === "bn" ? "bn-BD" : "en-US", { weekday: "long", day: "numeric", month: "long" })}
            </div>
          </div>

          {loading ? (
            <div className="flex flex-col items-center justify-center py-16"><Loader2 className="w-12 h-12 animate-spin text-amber-400" /></div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
              {schoolSchedule.map((item, idx) => (
                <div key={idx} className="bg-white/5 p-8 rounded-[2.5rem] text-center space-y-4 border border-white/5 hover:bg-white/10 transition-all group">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg font-bold text-white">{language === "bn" ? item.nameBn : item.nameEn}</h3>
                    <p className="text-sm font-black text-amber-400 uppercase tracking-tighter">{item.time}</p>
                  </div>
                  <div className="pt-3 border-t border-white/5">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">{language === "bn" ? item.tagBn : item.tagEn}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          <div className="lg:col-span-2 flex flex-col md:flex-row gap-12 items-center md:items-start bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-blue-900/5">
            ...
            <div className="space-y-6">
              <div className="inline-block px-4 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-black uppercase tracking-widest">Message from Principal</div>
              <h2 className="text-4xl font-black text-slate-900 leading-tight">শিক্ষা ও শৃঙ্খলাই আমাদের মূল শক্তি</h2>
              <p className="text-slate-500 text-lg leading-relaxed italic">"একটি সমৃদ্ধ জাতি গঠনে সঠিক শিক্ষার কোনো বিকল্প নেই। আমরা আমাদের প্রতিটি শিক্ষার্থীকে নৈতিকতা ও আধুনিক জ্ঞানের সমন্বয়ে গড়ে তুলতে প্রতিশ্রুতিবদ্ধ।"</p>
              <div>
                <h4 className="font-black text-slate-900 text-xl">কর্নেল এম. এ. রহমান</h4>
                <p className="text-blue-600 font-bold uppercase tracking-widest text-xs">অধ্যক্ষ, ক্যান্টনমেন্ট স্কুল</p>
              </div>
            </div>
          </div>
      ...
      <section className="bg-blue-950 py-24 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20 space-y-4">
            <h2 className="text-amber-400 font-black uppercase tracking-widest text-sm">Why Choose Us</h2>
            <h2 className="text-4xl md:text-5xl font-black">আমাদের বিশেষত্বসমূহ</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {deptCards.map((dept, index) => (
              <motion.div key={index} className="group bg-white/5 rounded-[2.5rem] p-8 border border-white/10 hover:bg-white hover:text-slate-900 transition-all duration-500">
                <div className={`w-16 h-16 ${dept.color} rounded-2xl flex items-center justify-center mb-8 shadow-lg`}>
                  <dept.icon className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-2xl font-black mb-4">{language === "bn" ? dept.titleBn : dept.titleKey}</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-12">
          <h2 className="text-3xl font-black text-slate-900">{language === "bn" ? "আসন্ন ইভেন্ট" : "Upcoming Events"}</h2>
          <Link href="/events" className="text-blue-600 font-bold flex items-center gap-2">View All <ArrowRight className="w-4 h-4" /></Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {events.map((event, index) => (
            <motion.div key={index} whileHover={{ y: -5 }} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 flex items-center gap-8 shadow-lg shadow-blue-900/5">
              <div className="bg-blue-600 text-white p-6 rounded-[1.5rem] text-center min-w-[100px]">
                <span className="block text-3xl font-black">{language === "bn" ? event.date.split(" ")[0] : event.dateEn.split(" ")[1]}</span>
                <span className="text-xs font-bold uppercase tracking-widest opacity-80">{language === "bn" ? event.date.split(" ")[1] : event.dateEn.split(" ")[0]}</span>
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-slate-900">{language === "bn" ? event.titleBn : event.titleEn}</h3>
                <div className="flex items-center gap-4 text-sm font-bold text-slate-400">
                  <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> {language === "bn" ? event.timeBn : event.timeEn}</span>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div className="relative bg-gradient-to-br from-blue-600 to-blue-800 rounded-[3rem] overflow-hidden px-8 py-20 text-white text-center shadow-2xl shadow-blue-600/20">
          <div className="relative z-10 space-y-8 max-w-3xl mx-auto">
            ...
            <h2 className="text-4xl md:text-6xl font-black leading-tight">আপনার সন্তানের উজ্জ্বল ভবিষ্যৎ শুরু হোক এখান থেকেই</h2>
            <p className="text-white/80 text-xl font-medium">২০২৬ শিক্ষাবর্ষে প্লে থেকে ৯ম শ্রেণী পর্যন্ত সীমিত আসনে ভর্তি চলছে।</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/admission" className="bg-amber-400 hover:bg-amber-500 text-blue-950 px-12 py-5 rounded-[2rem] font-black text-xl transition-all shadow-2xl">এখনই আবেদন করুন</Link>
              <button className="bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 px-12 py-5 rounded-[2rem] font-black text-xl transition-all">যোগাযোগ করুন</button>
            </div>
          </div>
        </motion.div>
      </section>
